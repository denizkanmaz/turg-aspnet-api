...
namespace Turg.App
{
    public class Product
    {
        ...
        public static async Task<List<Product>> GetByCategoryId(string categoryId)
        {
            await using var conn = new NpgsqlConnection(Constants.ConnectionString);
            await conn.OpenAsync();
            ...
            await using (var cmd = new NpgsqlCommand($"SELECT * FROM products WHERE category = '{categoryId}'", conn))
            {
                await using NpgsqlDataReader reader = await cmd.ExecuteReaderAsync();
        ...
        public static async Task<Guid> Create(Product product)
        {
            await using var conn = new NpgsqlConnection(Constants.ConnectionString);
            await conn.OpenAsync();
